...
void printArray(CATEGORY *arrpArray[], int arrptr_size, struct cart **head) 
{
	int iid, quantity;
...
int main()
{
    ...
    FILE *file = fopen("final_file.csv", "r");
    if (!file)
	{
	...
	int ua,u_choice,a_choice,select;
						...
						{	
							while (1) 
							{
								...
								if (select >= 1 && select <= arrptr_size)
								{
									...
									printArray(arrpArray, select - 1, &head_cart);
								} 
								else 
								{
